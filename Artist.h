...
class Artist : public virtual Record{
    string rec;
    int x;
    public:
    void set( string);
...
void Artist :: set( string rec){
    x = 0;
    this -> rec = rec;
}

void Artist :: showData()
{
    fstream fin;
...
int Artist :: Delete( string name,  int x)
{
    int del = 0;
    fstream fin, fout;
    fin.open("Database - Sheet1.csv", ios::in);
...
void Artist :: Update( string name,  int x)
{
    fstream fin, fout;
    fin.open("Database - Sheet1.csv", ios::in);
    fout.open("New.csv", ios::out);
    ...
    if (count == 0)
        ...
        cout << "Record has been streamed successfully!\n";
    }
    fin.close();
    fout.close();
    remove("Database - Sheet1.csv");
